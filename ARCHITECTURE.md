...
`wickra-feature-store` is one data-driven core with many thin consumers. A screen is a
...
CONSUMERS   CLI: crates/feature-store-cli        ·   any language via its binding (command JSON)
      ▲ ScanReport JSON                                      ▲
CORE  crates/feature-store-core:  ScanSpec (JSON) → Universe<Symbol, SymbolState> (O(1)/bar)
...
Each binding ships the same surface — a `Feature-Store` handle plus
...
`Feature-Store::command`. The binding does no logic of its own — it forwards the
...
resolver the backtester uses), so the feature-store inherits all 514 indicators and
any future additions for free. Price fields read straight from the candle.

## Integration with the rest of Wickra

`wickra-feature-store` sits beside the other Wickra consumers — the terminal, the
