...
make, so this reads `wickra_feature_store_<name>` out of
`bindings/c/include/wickra_feature_store.h` and checks each language's public surface
...
HEADER = os.path.join(ROOT, "bindings", "c", "include", "wickra_feature_store.h")

# Exports that are ABI plumbing rather than a promise to callers.
ABI_ONLY = {"free"}

EXPORT = re.compile(r"\bwickra_feature_store_([a-z0-9_]+)\s*\(")
...
BINDINGS: dict[str, tuple[list[str], dict[str, str]]] = {
    "python": (
        ["bindings/python/src/lib.rs", "bindings/python/python/wickra_feature_store/__init__.py"],
        {
            "new": r"(?m)^\s*fn new\s*\(",
...
def main() -> int:
    ...
    if not exports:
        print(f"no wickra_feature_store_* exports found in {HEADER}", file=sys.stderr)
        return 1
