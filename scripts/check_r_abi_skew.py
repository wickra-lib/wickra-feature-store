...
takes the header and library from `WKFEATURESTORE_INC` / `WKFEATURESTORE_LIB`, supplied from
outside, and compiles the hand-written `src/wickra_feature_store.c` against whatever
...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_feature_store.c` calls must be declared in
    `bindings/c/include/wickra_feature_store.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-feature-store-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_feature_store.h"
