...
h <- wkfeaturestore_new('{"universe":["AAA"],"features":[{"kind":"indicator","name":"Sma","params":[2]},{"kind":"price","field":"close"}],"labels":[{"kind":"forward_return","horizon":1}]}')
out <- wkfeaturestore_command(h, '{"cmd":"version"}')
stopifnot(is.character(out), length(out) == 1L)
stopifnot(grepl("version", out, fixed = TRUE))

cat("wickra-feature-store R package smoke: ok (version ", v, ")\n", sep = "")
