//! The wickra-feature-store C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `FeatureStore::command_json`: construct a `FeatureStore` from a spec JSON,
//! drive it with command JSONs and read back response JSONs, and free the
//! handle. No feature-store type crosses the boundary by value — the handle is
...
//!
//! 1. Call [`wickra_feature_store_command`] with `out = NULL`, `cap = 0` to learn
...
//! for unusable arguments ([`WICKRA_FEATURE_STORE_ERR_NULL`],
//! [`WICKRA_FEATURE_STORE_ERR_UTF8`]) and caught panics
//! ([`WICKRA_FEATURE_STORE_ERR_PANIC`]); a non-negative return is always the
...
pub const WICKRA_FEATURE_STORE_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_FEATURE_STORE_ERR_PANIC: i32 = -3;
...
#[no_mangle]
pub unsafe extern "C" fn wickra_feature_store_new(
    spec_json: *const c_char,
) -> *mut WickraFeatureStore {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_feature_store_free(handle: *mut WickraFeatureStore) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_feature_store_command(
    handle: *mut WickraFeatureStore,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_FEATURE_STORE_ERR_NULL;
    }
...
#[no_mangle]
pub extern "C" fn wickra_feature_store_version() -> *const c_char {
...
mod tests {
    ...
    const SPEC: &str = r#"{"universe":["AAA"],"features":[{"kind":"price","field":"close"}],"labels":[{"kind":"forward_return","horizon":1}]}"#;
    ...
    fn new_command_free_round_trip() {
        ...
        unsafe { wickra_feature_store_free(handle) };
    }

    #[test]
    fn build_batch_returns_a_matrix() {
        let spec = CString::new(SPEC).unwrap();
        let handle = unsafe { wickra_feature_store_new(spec.as_ptr()) };
        let cmd = CString::new(
        ...
        assert!(read_buf(&buf).contains("\"price.close\""));
        unsafe { wickra_feature_store_free(handle) };
    }
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_feature_store_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_spec_yields_null_handle() {
        let handle = unsafe { wickra_feature_store_new(ptr::null()) };
        assert!(handle.is_null());
    }
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code = unsafe {
            wickra_feature_store_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0)
        };
        assert_eq!(code, WICKRA_FEATURE_STORE_ERR_NULL);
        // Null command with a valid handle.
        let spec = CString::new(SPEC).unwrap();
        let handle = unsafe { wickra_feature_store_new(spec.as_ptr()) };
        let code = unsafe { wickra_feature_store_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_FEATURE_STORE_ERR_NULL);
        unsafe { wickra_feature_store_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_feature_store_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_feature_store_version();
