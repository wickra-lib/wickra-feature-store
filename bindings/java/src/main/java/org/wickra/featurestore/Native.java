...
/** Raw FFM (Panama) downcall surface for the wickra-feature-store C ABI. */
final class Native {
    private Native() {}
    ...
    static final MethodHandle NEW =
            handle("wickra_feature_store_new", FunctionDescriptor.of(C_PTR, C_PTR));
    static final MethodHandle FREE =
            handle("wickra_feature_store_free", FunctionDescriptor.ofVoid(C_PTR));
    static final MethodHandle COMMAND =
            handle("wickra_feature_store_command", FunctionDescriptor.of(C_INT, C_PTR, C_PTR, C_PTR, C_LONG));
    static final MethodHandle VERSION =
            handle("wickra_feature_store_version", FunctionDescriptor.of(C_PTR));
    ...
    private static SymbolLookup loadLibrary() {
        String libFile = System.mapLibraryName("wickra_feature_store");
        ...
        throw new UnsatisfiedLinkError("wickra-feature-store: could not load the native library (" + libFile
    ...
    private static Path extractBundled(String libFile) {
        ...
        try (InputStream in = Native.class.getResourceAsStream(resource)) {
            ...
            Path tmp = Files.createTempFile("wickra-feature-store-", "-" + libFile);
