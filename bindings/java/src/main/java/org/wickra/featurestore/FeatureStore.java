 ...
 * ({@code set_spec}, {@code push}, {@code push_batch}, {@code build},
 * {@code build_batch}, {@code labels}, {@code reset}, {@code version}) and read
...
public final class FeatureStore implements AutoCloseable {
     ...
     *
     * @throws IllegalArgumentException if the spec is not a valid feature-store spec
     */
    public FeatureStore(String specJson) {
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (created.address() == 0) {
                throw new IllegalArgumentException("wickra-feature-store: invalid spec");
            }
            this.handle = created;
    ...
    public String command(String cmdJson) {
        if (handle == null) {
            throw new IllegalStateException("feature store is closed");
        }
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment cmd = arena.allocateFrom(cmdJson);
            int len = (int) Native.COMMAND.invokeExact(handle, cmd, MemorySegment.NULL, 0L);
            if (len < 0) {
                throw new IllegalStateException("wickra-feature-store: command failed (code " + len + ")");
            }
    ...
    /** Free the native feature-store handle. */
    @Override
    public void close() {
        if (handle != null) {
            try {
