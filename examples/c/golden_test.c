 ...
 *
 * Build the store from each committed golden/specs/*.json, build a matrix over
...
int main(void) {
    ...
    for (size_t i = 0; i < GOLDEN_SPEC_COUNT; i++) {
        ...
        char *command = join("{\"cmd\":\"build_batch\",\"data\":", dataset, "}");
        WickraFeatureStore *store = command ? wickra_feature_store_new(spec) : NULL;
        if (!store) {
        ...
        int32_t len = wickra_feature_store_command(store, command, NULL, 0);
        char *got_raw = len >= 0 ? (char *)malloc((size_t)len + 1) : NULL;
        if (got_raw) {
            wickra_feature_store_command(store, command, got_raw, (size_t)len + 1);
