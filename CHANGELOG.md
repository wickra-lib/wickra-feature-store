...
A maintenance release: the feature store and its bindings are unchanged. It
...
- **Third-party dependencies refreshed.** `Cargo.lock` takes 61 crates to their
...
- **The Go install line names the published module.** The badge and `go get`
  ...
  `github.com/wickra-lib/wickra-feature-store/bindings/go`, which the Go proxy never
  serves; they now name the mirror `github.com/wickra-lib/wickra-feature-store-go`, as the
...
- **wickra-backtest-core 0.1.6.** The pin moves from `=0.1.4` to the release
  ...
  repositories up, and the rest is what this one spelled differently: the fuzz
  job runs the family's pinned `nightly-2026-07-01` rather than a rolling
  nightly, and the example job's `dotnet-version` reads `8.0.x`.
...
[Unreleased]: https://github.com/wickra-lib/wickra-feature-store/compare/v0.1.3...HEAD
[0.1.3]: https://github.com/wickra-lib/wickra-feature-store/compare/v0.1.2...v0.1.3
